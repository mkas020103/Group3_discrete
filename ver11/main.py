...
class GUI:
    def __init__(self):
        ...
        # Create a temporary list for undo and redo
        self.temp_list_of_pictures = []
        ...
        # Set App Icon
        self.root.iconbitmap(r'filterio.ico')
        ...
        self.blur.grid(row=0, column=0, padx=22, pady=(70, 5))
        ...
        # Create sepia button
        self.sepia = Button(self.left_frame, text="Sepia", font=('Arial', 11), command=self.sepia)
        self.sepia.grid(row=7, column=0, padx=22, pady=(5, 5))

        # Create blue button
        self.blue = Button(self.left_frame, text="Blue", font=('Arial', 11), command=self.blue)
        self.blue.grid(row=8, column=0, padx=22, pady=(5, 5))

        # Create reset button
        self.reset = Button(self.left_frame, text="RESET", font=('Arial', 11), command=self.reset)
        self.reset.grid(row=9, column=0, padx=22, pady=(100, 5))
        ...
        # Ask user for the filter
        self.instruction_filter = Label(master=self.left_frame, text="Choose Filter", font=("Arial", 11), bg="#404040",
                                 fg="white")
        self.instruction_filter.pack(padx=22, pady=(30, 1))
        ...
        self.previous_label = Label(master=self.right_frame, text="Previous Image", font=("Arial", 11), bg="#404040",
                                    fg="white")
        self.previous_label.pack(padx=20, pady=(15, 1))

        # Create a save button
        self.add_save_img = ImageTk.PhotoImage(
            Image.open(r'Save Button.png').resize((30, 30), Image.ANTIALIAS))
        self.save_btn = Button(master=self.right_frame, image=self.add_save_img, bg="#404040", borderwidth=0,
                               command=self.save)
        self.save_btn.place(relx=0.5, rely=1, anchor='s', y=-150, x=-60)

        # Create a undo button
        self.add_undo_img = ImageTk.PhotoImage(
            Image.open(r'Undo Button.png').resize((30, 30), Image.ANTIALIAS))
        self.undo_btn = Button(master=self.right_frame, image=self.add_undo_img, bg="#404040", borderwidth=0,
                               command=self.undo)
        self.undo_btn.place(relx=0.5, rely=1, anchor='s', y=-150)

        # Create a redo button
        self.add_redo_img = ImageTk.PhotoImage(
            Image.open(r'Redo Button.png').resize((30, 30), Image.ANTIALIAS))
        self.redo_btn = Button(master=self.right_frame, image=self.add_redo_img, bg="#404040", borderwidth=0,
                               command=self.redo)
        self.redo_btn.place(relx=0.5, rely=1, anchor='s', y=-150, x=60)

        # Create a quit button
        self.quit = Button(self.right_frame, text="QUIT", font=("Arial", 11), command=self.root.destroy, bg="#F20000",
                           fg="white")
        self.quit.place(relx=0.5, rely=1, anchor='s', y=-80)
    ...
    def error_message(self):
        ...
        self.warning = Label(self.center_frame, text="INPUT AN IMAGE FILE PATH!", fg="red", font="Arial 20 bold",
                             bg="#262626")
        self.warning.pack()
    ...
    def sepia(self):
        try:
            ...
            # sepia filter
            self.last_image_copy = self.last_image.copy()
            width, height = self.last_image_copy.size
            pixels = self.last_image_copy.load()  # create the pixel map
            for py in range(height):
                for px in range(width):
                    r, g, b = self.last_image_copy.getpixel((px, py))
                    tr = int(0.600 * r + 0.769 * g + 0.189 * b)
                    tg = int(0.450 * r + 0.686 * g + 0.168 * b)
                    tb = int(0.380 * r + 0.534 * g + 0.131 * b)
                    if tr > 255:
                        tr = 255
                    if tg > 255:
                        tg = 255
            ...
            self.filter = self.last_image_copy.convert("RGB")
            self.append_display_destroy()
        except StopIteration:
            self.error_message()
    ...
    def reset(self):
        try:
            ...
            # print undo error
                self.filter_added = Label(self.center_frame, text="There's nothing to reset!", fg="#F20000",
                                          font=("Abadi", 15), bg="#262626")
                self.filter_added.place(relx=0.5, rely=1, anchor='s', y=-35)
                self.root.after(1000, self.filter_added.destroy)

    # save button
    def save(self):
        if not self.list_of_pictures:
            self.error_message()
        else:
            try:
                files = [('All Types', '*.*'),
                ...
                # get the file path
                save_filepath = filedialog.asksaveasfilename(filetypes=files, defaultextension='.png')
                # save the image in the chosen path
                self.list_of_pictures[self.right_picture].resize((int(self.width), int(self.height))).save(save_filepath)
            except ValueError:
                # print undo error
                self.filter_added = Label(self.center_frame, text="", fg="#F20000",
                                          font=("Abadi", 15), bg="#262626")
                self.filter_added.place(relx=0.5, rely=1, anchor='s', y=-35)
                self.root.after(1000, self.filter_added.destroy)
    ...
    # undo button
    def undo(self):
            # if there are mora than one added filters
            if len(self.list_of_pictures) > 2:
                # add the last added filter to the temporary list
                temp_var = self.list_of_pictures.pop()
                self.temp_list_of_pictures.append(temp_var)

                # get the last image in the list
                self.last_image = self.list_of_pictures[-2]
                self.filter = self.list_of_pictures[-1]
                temp_var = self.list_of_pictures.pop()
                self.append_display_destroy()

                # decrement right_picture
                self.right_picture -= 2

            # if there is only one added filter
            elif len(self.list_of_pictures) == 2:
                # add the last added filter to the temporary list
                temp_var = self.list_of_pictures.pop()
                self.temp_list_of_pictures.append(temp_var)

                # remove the picture on the right and center frame
                self.previous.destroy()
                self.displaying_image.destroy()

                # display the latest image
                self.current_filter = ImageTk.PhotoImage(self.list_of_pictures[-1])
                self.displaying_image = Label(master=self.center_frame, image=self.current_filter)
                self.displaying_image.pack()
            ...
            # No more filters, undo error
            else:
                # print undo error
                self.filter_added = Label(self.center_frame, text="No more added filters to remove", fg="#F20000",
                                          font=("Abadi", 15), bg="#262626")
                self.filter_added.place(relx=0.5, rely=1, anchor='s', y=-35)
                self.root.after(1000, self.filter_added.destroy)
    ...
    # redo button
    def redo(self):
        if len(self.temp_list_of_pictures) > 0:
            # get the last image in the list
            self.last_image = self.list_of_pictures[-1]

            # return the last added filter of temporary list
            temp_var = self.temp_list_of_pictures.pop()
            x = self.list_of_pictures.pop()
            self.list_of_pictures.append(temp_var)
            self.filter = self.list_of_pictures[-1]
            self.append_display_destroy()

        # No more filters, redo error
        else:
            self.filter_added = Label(self.center_frame, text="No more removed filters to add", fg="#F20000",
                                      font=("Abadi", 15), bg="#262626")
            self.filter_added.place(relx=0.5, rely=1, anchor='s', y=-35)
            self.root.after(1000, self.filter_added.destroy)
